...
\AR{We thank the reviewer for this important observation. Parameter selection is indeed crucial for practical implementation, and we have substantially enhanced the manuscript with comprehensive parameter analysis and selection strategies.}

\textbf{Comprehensive Parameter Sensitivity Analysis:}

We added a new \Cref{subsec:parameter-sensitivity-analysis} `Parameter Sensitivity Analysis' that provides systematic analysis of all core hyperparameters:

\begin{itemize}
  \item \textbf{Block Size Optimization:} Systematic analysis of partition configurations from $5 \times 5$ to $14 \times 14$ blocks, identifying 100 partitions ($10 \times 10$) as the optimal configuration that minimizes computation time (512 seconds) while maintaining clustering quality
  \item \textbf{Threshold Parameter Analysis:} Comprehensive evaluation of co-cluster size thresholds $T_m, T_n \in \{5, 10, 15, 20, 30, 50, 75, 100\}$, demonstrating robust performance across the range [10, 50] with optimal balance at $T_m = T_n = 20$
  \item \textbf{Probability Threshold Study:} Analysis of $P_{\text{thresh}} \in \{0.80, 0.85, 0.90, 0.95, 0.99\}$ showing stable performance above 0.95, with 0.95 providing the best trade-off between detection accuracy and computational efficiency
  \item \textbf{Parameter Interaction Analysis:} 3D surface plots and factorial experiments revealing that clustering quality is most sensitive to joint tuning of $P_{\text{thresh}}$ and $T_m$, while partition count primarily affects runtime
\end{itemize}

\textbf{Practical Parameter Selection Guidelines:}

Based on our comprehensive analysis, we provide actionable recommendations for different scenarios:

\begin{quote}
  \textbf{Default Configuration:} Use 100 partitions ($10 \times 10$), $T_m = T_n = 20$, and $P_{\text{thresh}} = 0.95$ for general applications.

  \textbf{High-Precision Applications:} Increase $P_{\text{thresh}}$ to 0.99 for applications requiring high confidence in co-cluster detection.

  \textbf{Large-Scale Processing:} Ranges from 81–121 partitions retain $>95\%$ quality while offering flexibility in computational resource allocation.

  \textbf{Small Co-cluster Detection:} Reduce $T_m = T_n$ to 10–15 for datasets where smaller co-clusters are expected to be meaningful.
\end{quote}

\textbf{Robustness Validation:}

Our analysis demonstrates that DiMergeCo exhibits robust performance across parameter ranges:
- Quality metrics (NMI/ARI) remain within 5\% of optimal across the recommended parameter ranges
- The method gracefully degrades rather than failing catastrophically when parameters deviate from optimal values
- Parameter transferability across different datasets and domains (as shown in our multi-domain validation) reduces the need for extensive per-dataset tuning

\textbf{Manuscript Modifications:}

We enhanced the manuscript with:

\begin{quote}
  \textbf{Section 6.3 Parameter Sensitivity Analysis} includes:
  - \textbf{Subsection 6.3.1:} Partition Configuration Optimization with Figure 4 showing the optimization landscape
  - \textbf{Subsection 6.3.2:} Individual Parameter Sensitivity with detailed analysis of each hyperparameter
  - \textbf{Subsection 6.3.3:} Parameter Interaction Analysis examining joint effects
  - \textbf{Subsection 6.3.4:} Practical Parameter Selection Guidelines providing actionable recommendations

  This comprehensive analysis spans 2 pages with 3 new figures and 1 supplementary table, providing complete guidance for parameter selection and tuning in practical applications.
\end{quote}

The analysis confirms that while parameter selection affects performance, DiMergeCo's design ensures robust operation across reasonable parameter ranges, making it practical for deployment without extensive hyperparameter optimization.
