...
%%%
 ...
 % Author: Zihan
 % -----
 % Last Modified: Thursday, 21st December 2023 11:09:11 am
...
Our method concentrates on coclustering on large-scale data. The main idea is to divide the data into blocks, do coclustering on each block, and then ensemble the results. The algorithm is shown in Algorithm \ref{alg:method}.

\begin{algorithm}[!t]
    \caption {Parallel Co-clustering Method}\label{alg:method}
    \begin{algorithmic}[1]
        \REQUIRE {Source data $\mathcal{X}^s$, Source labels ${\bf y}$, Target data $\mathcal{X}^t$, Batch-size $b_s$, Maximum iteration $T_{max}$, Update iteration $T_{up}$, Learning rate $\lambda$, Parameter $\lambda_1$ and $\lambda_2$;}
        \ENSURE {Learned network $\bm{\Theta}$, Target prediction $\hat{\bf y}^t$;}\\
        \STATE 
    \end{algorithmic}
\end{algorithm}
