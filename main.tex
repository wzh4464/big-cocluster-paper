...
%%%
 ...
 % Author: Zihan
 % -----
 % Last Modified: Saturday, 8th June 2024 1:43:48 am
...
\begin{itemize}
  % \item The time complexity for each submatrix, given by the atom co-clustering method, is \(O(f(n))\). By partitioning the matrix and distributing the submatrices across multiple nodes, each node only requires time \(O(f(\phi_i \times \psi_j))\). For the merging process, 
  \item \emph{Partitioning and Co-clustering on Submatrices:} We only do $O(1)$ computation for partitioning step, and by distributing the submatrices across multiple nodes, each node only requires time \(O(f(\phi_i \psi_j))\).
  \item \emph{Hierarchical Merging:} The hierarchical merging process
\end{itemize}
...
Similarly, the space complexity for each submatrix, given by the atom co-clustering method, is \(O(g(n))\). By partitioning the matrix and distributing the submatrices across multiple nodes, each node only requires space \(O(g(\phi_i \times \psi_j))\). The overall space complexity for the framework is the maximum space complexity among the submatrices:
\[ O\left(\max_{i,j} g(\phi_i \times \psi_j)\right) \]
