...
%%%
 ...
 % Author: Zihan
 % -----
 % Last Modified: Tuesday, 4th June 2024 12:03:30 pm
...
In the following subsections, we first introduce the large matrix partitioning algorithm, which is the cornerstone of our scalable co-clustering method. We then present the atom-co-clustering algorithm, which is compatible with our framework and can be applied to each submatrix. Finally, for the self-containedness of this paper, we introduce a graph-based spectral co-clustering algorithm, which can be integrated into our framework and applied to each submatrix.
