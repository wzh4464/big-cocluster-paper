...
%%%
 ...
 % Author: Zihan
 % -----
 % Last Modified: Saturday, 8th June 2024 12:50:35 pm
...
% % definition of Mahalanobis distance Distance
% \begin{definition}[Mahalanobis Distance]
%   \label{def:mahalanobis_distance}
%   Consider a point \( x \in \mathbb{R}^n \) and a covariance matrix \( \Sigma \in \mathbb{R}^{n \times n} \). The Mahalanobis distance of \( x \) is defined as:
%   \[
...
% The Mahalanobis distance is a measure of the distance between a point and a distribution, taking into account the covariance structure of the data. If one considered $(s_1, s_2)$ as a point in a two-dimensional space, the Mahalanobis distance would measure the distance of this point from the origin, taking into account the covariance structure of the data. This distance is particularly useful for identifying outliers or anomalies in high-dimensional data, as it accounts for the correlation between variables.
...
\paragraph{Hierarchical Merging} The hierarchical merging process iteratively combines co-clusters from the submatrices. This can be represented by a binary tree, where each internal node corresponds to a merged co-cluster, and each leaf node represents a small co-cluster. The time complexity of the hierarchical merging process is upbounded by \(O(n \log n \cdot L(\phi_\text{max}, \psi_\text{max}))\), where \(L(\phi_\text{max}, \psi_\text{max})\) is the time complexity of calculating the score of merging two co-clusters with the largest size \(\phi_\text{max},  \psi_\text{max}\). Using the methods from \cite{lee2014BigDataMatrix}, we have \(L(\phi_\text{max}, \psi_\text{max}) = O(16 \max(\phi_\text{max}, \psi_\text{max})) = O(\max(\phi_\text{max}, \psi_\text{max}))\).

Combining these, the overall time complexity for our method can be upbounded by:
\begin{align}
...
\Cref{thm:probability_co_cluster_detection} can be used to set $m, n, \phi, \psi, T_m, T_n$, and $T_p$ to ensure the probability of detecting the co-cluster is larger than a given threshold.
...
\begin{definition}
  Suppose $\mathbf{A} \in \mathbb{R}^{M \times N}$ is a matrix, and $I \subseteq \{1, 2, \ldots, M\}$ and $J \subseteq \{1, 2, \ldots, N\}$ are two sets of row and column indices.
  \label{def:co_cluster_score}
  % two definitions: 1. matrix 2. submatrix
  \begin{enumerate}
...
\Cref{thm:expected_score} establishes the expected behavior of the score function $S(I,J)$ under noise, providing insights into how noise impacts the similarity measures used in our co-clustering method.
