...
%%%
 ...
 % Author: Zihan
 % -----
 % Last Modified: Thursday, 6th June 2024 5:05:56 pm
...
\begin{definition}[Singular Value Ratio Score]
  ...
  Consider a matrix \(\mathbf{A} \in \mathbb{R}^{M \times N}\), and let \(I\) and \(J\) denote the sets of row and column indices, respectively, for a submatrix \(\mathbf{A}_{I, J}\). The Singular Value Ratio Score is defined as follows:
  \begin{itemize}
    \item For a given submatrix \(\mathbf{A}_{I, J}\), the Singular Value Ratio Score is given by:
          \[
            s(\mathbf{A}_{I, J}) = \frac{s_1}{s_2},
          \]
          where \(s_1\) is the largest singular value of \(\mathbf{A}_{I, J}\) and \(s_2\) is the second largest singular value of \(\mathbf{A}_{I, J}\).
    \item For a given submatrix \(\mathbf{A}_{I, J}\), the normalized Singular Value Ratio Score is defined as:
          \[
            S(\mathbf{A}_{I, J}) = \frac{s(\mathbf{A}_{I, J})}{\|\mathbf{A}_{I, J}\|_F},
          \]
          where \(\|\mathbf{A}_{I, J}\|_F\) denotes the Frobenius norm of \(\mathbf{A}_{I, J}\).
  \end{itemize}
\end{definition}
