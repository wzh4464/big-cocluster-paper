...
\begin{proof}[Proof of \Cref{thm:joint-probability}]
    Let $M_{(i,j)}^{(k)}$ be the number of rows of co-cluster $C_k$ that fall into the $i$-th row partition of size $\phi_i$. Under the random partitioning model (\Cref{assumption:random-partitioning}), this count follows a hypergeometric or multinomial distribution (depending on the sampling procedure). The expected number of rows is $E[M_{(i,j)}^{(k)}] = M^{(k)} \phi_i / M$.

    For concentration bounds, we note that hypergeometric random variables exhibit negative correlation, which makes them ``more concentrated'' than independent Bernoulli variables. Consequently, concentration inequalities like Hoeffding's or Serfling's inequality provide valid (and often conservative) bounds. We apply a Hoeffding-type bound for sampling without replacement:
    \begin{equation}
        \begin{aligned}
...
\begin{proof}[Proof of \Cref{thm:probability-co-cluster-detection}]
    Let $\mathcal{F}_{k,t}$ be the event that co-cluster $C_k$ is missed in partition trial $t$. A co-cluster is missed if it is not detected in \textbf{any} of the $m \times n$ blocks. Detection in a block $(i,j)$ fails if $M_{(i,j)}^{(k)} < T_m$ \emph{or} $N_{(i,j)}^{(k)} < T_n$. To obtain a tractable upper bound, we use the stricter (hence more probable) event where \emph{both} dimensions are below the threshold simultaneously: $M_{(i,j)}^{(k)} < T_m$ \emph{and} $N_{(i,j)}^{(k)} < T_n$. This gives us a conservative bound. The event $\mathcal{F}_{k,t}$ occurs if this holds for all blocks: $\mathcal{F}_{k,t} \supseteq \bigcap_{i=1}^m \bigcap_{j=1}^n \{M_{(i,j)}^{(k)} < T_m \text{ and } N_{(i,j)}^{(k)} < T_n\}$.
