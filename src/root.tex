...
% Unified notation macros for consistency
...
In the scenario where the matrix $\mathbf{A}$ is partitioned into $m \times n$ blocks, each block has size $\phi_i \times \psi_j$, that is, $M=\sum_{i=1}^m \phi_i$ and $N=\sum_{j=1}^n \psi_j$, the hypergeometric tail bounds for row and column hits are given by~\Cref{lem:hyp-tail}:
\begin{equation}
    \begin{split}
...
To ensure theoretical rigor and facilitate reproducibility, we analyze partitioning under the following randomized block model.
...
The number of significant co-clusters $K$ is finite and bounded by $O(\min(M,N))$.
\end{assumption}
...
Since row partitioning is obtained by random permutation (Assumption~\ref{assump:perm-partition}),
$X_{i,t}^{(k)}$ is a hypergeometric random variable corresponding to sampling $\phi_i$ items
without replacement from a population of size $M$ containing $M^{(k)}$ successes.
A standard Hoeffding--Serfling inequality for sampling without replacement states that for any
$\varepsilon>0$,
\[
\Pr\!\left(\frac{X_{i,t}^{(k)}}{\phi_i}
\le \frac{M^{(k)}}{M} - \varepsilon \right)
\le \exp(-2\phi_i\varepsilon^2).
\]
Setting $\varepsilon = s_i^{(k)} = \left(\frac{M^{(k)}}{M}-\frac{T_m-1}{\phi_i}\right)_+$
(which is non-negative by the positive part) yields \eqref{eq:row-tail}.
See \Cref{appendix:proof-hyp-tail} in the appendix for the detailed derivation.
\end{proof}
}
...
$Y_{j^\star,t}^{(k)}\ge T_n$, then in block $(i^\star,j^\star)$, the co-cluster portion has
at least $T_m$ rows and $T_n$ columns, hence co-cluster $k$ is captured.
Taking complements yields the stated miss event.
\end{proof}
}

\begin{assumption}[Negative association across blocks]
\label{assump:NA}
...
This is a standard property of multivariate hypergeometric distributions.
\end{assumption}

\begin{theorem}[Single-trial miss bound]
\label{thm:single-trial-miss}
Under Assumptions~\ref{assump:perm-partition}--\ref{assump:NA}, for each co-cluster $k$ and trial $t$, define
\[
...
\]
Then the probability that co-cluster $k$ is missed in a single trial satisfies
\begin{equation}
\Pr(F_{k,t})
\le \bar p_{\mathrm{row}}^{(k)} + \bar p_{\mathrm{col}}^{(k)}.
\label{eq:single-trial-miss}
\end{equation}
\end{theorem}

{\color{blue}
\begin{proof}
By Lemma~\ref{lem:trial-equivalence}, the miss event is
$F_{k,t} = \{\forall i: X_{i,t}^{(k)}<T_m\} \cup \{\forall j: Y_{j,t}^{(k)}<T_n\}$.
Using the union bound and Assumption~\ref{assump:NA} on the row and column events separately,
we obtain \eqref{eq:single-trial-miss}.
See \Cref{appendix:proof-single-trial-miss} in the appendix for the detailed derivation.
\end{proof}
}

\begin{theorem}[Probability of detecting all co-clusters over $T_p$ trials]
...
\end{equation}
Moreover, plugging Theorem~\ref{thm:single-trial-miss} yields
$q_k \le \bar p_{\mathrm{row}}^{(k)} + \bar p_{\mathrm{col}}^{(k)}$,
providing explicit computable bounds.
\end{theorem}

{\color{blue}
\begin{proof}
For each $k$, by trial independence,
$\Pr(\text{miss }k\text{ in all }T_p\text{ trials}) = q_k^{T_p}$.
Applying the union bound over all $K$ co-clusters gives \eqref{eq:detect-all-general}.
The exponential form \eqref{eq:detect-all-exp} follows by writing $q_{\max}^{T_p} = \exp(-\gamma_{\min} T_p)$.
See \Cref{appendix:proof-detect-all} in the appendix for the complete proof.
\end{proof}
}

\begin{corollary}[Uniform block sizes]
\label{cor:uniform}
Under uniform block sizes $\phi=M/m$, $\psi=N/n$, Theorem~\ref{thm:single-trial-miss} gives
\[
q_k \le \exp(-2M(s^{(k)})^2) + \exp(-2N(t^{(k)})^2),
\]
where $s^{(k)}$ and $t^{(k)}$ are the row and column gap parameters (averaged over blocks).
Thus Theorem~\ref{thm:probability-co-cluster-detection} yields
\begin{equation}
...
{\color{blue}
\begin{proof}
When blocks are uniform, $\sum_i \phi_i (s_i^{(k)})^2 = m \phi (s^{(k)})^2 = M(s^{(k)})^2$,
and similarly for columns. The bound follows immediately from Theorem~\ref{thm:single-trial-miss}
and Theorem~\ref{thm:probability-co-cluster-detection}.
\end{proof}
}

Building on \Cref{lem:hyp-tail,lem:block-fail,lem:trial-equivalence}, \Cref{thm:probability-co-cluster-detection} provides a robust framework for estimating the probability of successfully detecting all significant co-clusters across multiple partitioning iterations. It enables us to set partitioning parameters that achieve a desired success rate, thereby ensuring that the co-clustering process remains both reliable and efficient.

These theoretical results form the basis of our partitioning strategy by providing a mathematical foundation for selecting block sizes and determining the number of required partitioning iterations. The partitioning strategy preserves the essential low-rank structures within the data (supported by \Cref{thm:rank-monotonicity}), while the probabilistic bounds derived from \Cref{thm:probability-co-cluster-detection,cor:uniform} ensure that co-clusters are detected with high probability.
...
\begin{algorithm}[!t]
    ...
    \begin{algorithmic}[1]
        ...
        \STATE Compute $s^{(k)}, t^{(k)}$ via~\Cref{lem:hyp-tail} for all $C_k$
...
After obtaining local co-clusters, each block has produced multiple sub-co-clusters. To form a unified co-clustering solution for the entire data matrix, DiMergeCo merges overlapping or highly similar co-clusters using a hierarchical strategy guided by a regularized reconstruction error objective. {\color{blue}Our hierarchical merging strategy serves as a secondary protection layer, reconstructing co-clusters that may have been partially fragmented across block boundaries.}

\paragraph{Merging Objective}
As established in \Cref{subsec:merging-theory}, the merging process optimizes the regularized objective $\widetilde J_\lambda(\Cset) = J(\Cset) + \lambda |\Cset|$, where $J(\Cset)$ is the reconstruction error defined in \eqref{eq:objective-J} and $\lambda > 0$ is the regularization parameter. A merge of two co-clusters $C_a$ and $C_b$ is accepted if and only if:
\begin{equation}
    \Delta J(C_a, C_b) \le \lambda,
\end{equation}
where $\Delta J(C_a, C_b)$ is computed using \Cref{lem:merge-cost}. This criterion ensures that merging improves (or does not worsen) the trade-off between reconstruction accuracy and model complexity.

\paragraph{Disjointness Convention}
Throughout the merging analysis, $\Cset$ is assumed to be a partition of matrix entries,
i.e., each $(i,j)$ belongs to exactly one co-cluster. Overlap-based heuristics (e.g., Jaccard similarity) are used only
to generate candidate pairs for merging; before evaluating $\Delta J$, overlapping entries are assigned
to a unique cluster to maintain the partition property. This ensures that the merge cost formula in
\Cref{lem:merge-cost} and the reconstruction error objective in \eqref{eq:objective-J} remain well-defined.

\paragraph{Implementation via Priority Queue}
The merging process is implemented using a priority queue that prioritizes candidate merge pairs by their potential gain:
\begin{equation}
    \text{gain}(C_a, C_b) = \lambda - \Delta J(C_a, C_b).
\end{equation}
Pairs with higher gain (i.e., smaller merge cost) are considered first. To avoid exhaustive pairwise comparisons, we use heuristic filters based on overlap, coherence, and density to identify candidate pairs:
\begin{itemize}[leftmargin=*]
    \item \textbf{Overlap threshold $\tau$:} Only consider pairs with $\text{overlap}(C_a, C_b) \ge \tau$, where overlap is measured as the Jaccard similarity of row and column indices. Empirically, $\tau \in [0.4, 0.5]$ balances merge quality and computational cost.
    \item \textbf{Coherence and density:} Prioritize pairs with similar coherence and density scores to avoid merging structurally dissimilar co-clusters.
\end{itemize}
These filters serve only to accelerate the algorithm; the final merge decision is always based on the regularized objective $\widetilde J_\lambda$.

\paragraph{Merge Operation}
The merge operation combines two co-clusters by taking the union of their element sets: $\text{merge}(C_a, C_b) = C_a \cup C_b$. For efficient computation, we maintain summary statistics for each co-cluster:
\begin{itemize}[leftmargin=*]
    \item $|C|$: number of elements,
    \item $S_C = \sum_{(i,j) \in C} a_{ij}$: sum of elements,
    \item $Q_C = \sum_{(i,j) \in C} a_{ij}^2$: sum of squared elements.
\end{itemize}
These statistics allow us to compute $\Delta J(C_a, C_b)$ in $O(1)$ time without accessing the full matrix. Upon merging, the statistics are updated as: $|C_{ab}| = |C_a| + |C_b|$, $S_{C_{ab}} = S_{C_a} + S_{C_b}$, $Q_{C_{ab}} = Q_{C_a} + Q_{C_b}$.

\paragraph{Convergence and Quality Guarantee}
By \Cref{lem:merge-monotone}, the merging process is guaranteed to terminate in at most $K_0 - 1$ steps, where $K_0$ is the initial number of co-clusters. Moreover, \Cref{thm:merge-final-bound} ensures that the final solution satisfies an explicit reconstruction error bound. This approach refines local co-cluster boundaries and eliminates redundancies arising from parallel partitioning, resulting in a final co-cluster set that offers a consistent, global view of the underlying data structure.
...
{\color{blue}
The theoretical foundation of DiMergeCo rests on three pillars: (1) a probabilistic guarantee that co-clusters are preserved during partitioning, (2) a bound on the quality of the global solution relative to the true optimum, and (3) a proof of convergence for the hierarchical merging process with explicit bounds on the final reconstruction error. This section formalizes these guarantees, establishing the mathematical soundness of our framework.

\paragraph{Objective Function and Co-Cluster Representation}
Throughout this section, we define the co-clustering quality using a reconstruction error objective. Following the partitioning and local co-clustering stages, the matrix is divided into non-overlapping blocks $\{B_{(i,j)}\}$, and each block produces a local co-clustering solution $\Cset_{(i,j)} = \{\Ck^{(i,j)}\}_k$. Each co-cluster $\Ck$ is represented as a set of matrix element indices $\Ck \subseteq I \times J$. The global collection before merging is $\Cset^{(0)} = \bigcup_{i,j} \Cset_{(i,j)}$, which forms a partition of the matrix elements.

For any co-cluster $C$ (we drop subscripts when the context is clear), we define its optimal constant reconstruction value as the mean:
\begin{equation}
    u_C = \frac{1}{|C|}\sum_{(p,q)\in C} a_{pq},
\end{equation}
and the reconstruction error objective for a collection of co-clusters $\Cset$ as:
\begin{equation}\label{eq:objective-J}
    J(\Cset) = \sum_{C\in\Cset} \sum_{(i,j)\in C} (a_{ij}-u_C)^2.
\end{equation}
This objective measures the sum of squared errors (SSE) when each co-cluster is reconstructed by its mean value. It is equivalent to the standard co-clustering objective stated in Eq.~\eqref{eq:co-clustering-objective} in the special case where co-clusters form a complete partition. For a specific block $(i,j)$, the local objective is denoted $J_{(i,j)}(\Cset_{(i,j)})$. A solution is $\epsilon$-suboptimal if its objective value is within $\epsilon$ of the optimal value: $J(\Cset) \le J(\Cset^*) + \epsilon$.
...
\subsection{Hierarchical Merging: Theoretical Foundation}
\label{subsec:merging-theory}
Having established that local co-clusters are preserved and their union provides a bounded global solution, we now analyze the hierarchical merging process. The key challenge is to ensure that merging not only reduces redundancy but also provides explicit guarantees on the final reconstruction error. We address this by introducing a regularized objective that trades off reconstruction error against model complexity, and proving that the merging process monotonically optimizes this objective.

\subsubsection{Merge Cost and Regularized Objective}
To quantify the cost of merging two co-clusters, we first establish a fundamental lemma that characterizes how the reconstruction error changes when merging.

\begin{lemma}[Merge Cost under Squared Loss]\label{lem:merge-cost}
Let $C_a$ and $C_b$ be two disjoint co-clusters and let $C_{ab}=C_a\cup C_b$.
Define $u_C = \frac{1}{|C|}\sum_{(i,j)\in C}a_{ij}$ and
$J(\Cset)=\sum_{C\in\Cset}\sum_{(i,j)\in C}(a_{ij}-u_C)^2$.
Then the increase in reconstruction error from merging is:
\begin{equation}
\Delta J(C_a,C_b)
:=J(\{C_{ab}\})-J(\{C_a,C_b\})
= \frac{|C_a||C_b|}{|C_a|+|C_b|}\,(u_{C_a}-u_{C_b})^2 \ge 0.
\end{equation}
\end{lemma}
\begin{proof}
    The proof follows from the variance decomposition formula for the sum of squared errors. See \Cref{appendix:proof-merge-cost} in the appendix for the detailed proof.
\end{proof}

This lemma reveals that merging two co-clusters generally increases the reconstruction error $J$ (except when their means are identical). To enable beneficial merges that reduce redundancy, we introduce a regularized objective that explicitly penalizes model complexity:
\begin{equation}\label{eq:regularized-objective}
    \widetilde J_\lambda(\Cset) = J(\Cset) + \lambda |\Cset|,
\end{equation}
where $\lambda > 0$ is a regularization parameter controlling the trade-off between reconstruction accuracy and the number of co-clusters. A merge is beneficial if it reduces (or does not increase) $\widetilde J_\lambda$, which occurs when:
\begin{equation}
    \Delta J(C_a,C_b) \le \lambda.
\end{equation}
This criterion ensures that we merge co-clusters when the increase in reconstruction error is outweighed by the reduction in model complexity.

\subsubsection{Monotonicity and Convergence Guarantees}
With the regularized objective defined, we now establish that the hierarchical merging process is guaranteed to converge.

\begin{lemma}[Monotone Descent and Termination of Hierarchical Merging]
\label{lem:merge-monotone}
Let $\Cset^{(0)}$ be the union of all local co-clusters before merging and
$K_0=|\Cset^{(0)}|<\infty$.
If each merge is accepted only when
$\widetilde J_\lambda(\Cset^{(t+1)})\le \widetilde J_\lambda(\Cset^{(t)})$,
then the sequence $\{\widetilde J_\lambda(\Cset^{(t)})\}_{t\ge 0}$ is non-increasing and
the algorithm terminates after at most $K_0-1$ merges.
\end{lemma}
\begin{proof}
Each accepted merge reduces $|\Cset|$ by exactly one and does not increase
$\widetilde J_\lambda$ by construction. Since $|\Cset|$ is finite and cannot
drop below $1$, at most $K_0-1$ merges can occur. See \Cref{appendix:proof-merge-monotone} for details.
\end{proof}

This lemma establishes that the merging process operates on a well-defined objective (rather than a heuristic composite score) and is guaranteed to terminate with a stable solution.

\subsubsection{Final Reconstruction Error Bound}
The critical question for the overall framework is: how does the final solution after merging compare to the global optimum? The following theorem provides an explicit answer by combining the bounds from partitioning, local optimization, and merging.

\begin{theorem}[Global Objective Bound After Hierarchical Merging]
\label{thm:merge-final-bound}
Let $\Cset^{(0)}=\bigcup_{i,j}\Cset_{(i,j)}$ be the union of local
co-clusterings with $K_0=|\Cset^{(0)}|$.
Assume the hierarchical merging produces $\Cset_{\mathrm{merge}}$ by repeatedly
merging two clusters and accepting the merge only if
$\widetilde J_\lambda(\Cset)$ does not increase.
Then
\begin{equation}\label{eq:merge-J-bound}
J(\Cset_{\mathrm{merge}}) \le J(\Cset^{(0)}) + \lambda (K_0-1).
\end{equation}
Moreover, if each local solution $\Cset_{(i,j)}$ is $\epsilon_{(i,j)}$-suboptimal and
\Cref{thm:global-solution-quality} holds, then
\begin{equation}\label{eq:merge-final-bound}
J(\Cset_{\mathrm{merge}})
\le J(\Cset^*) + \sum_{i,j}\epsilon_{(i,j)} + \lambda (K_0-1).
\end{equation}
\end{theorem}
\begin{proof}
Each accepted merge reduces $|\Cset|$ by 1 and satisfies
$J(\Cset^{(t+1)})\le J(\Cset^{(t)})+\lambda$ (from the definition of $\widetilde J_\lambda$ and \Cref{lem:merge-cost}).
Summing over at most $K_0-1$ merges yields \eqref{eq:merge-J-bound}.
The bound \eqref{eq:merge-final-bound} follows by substituting the bound of $J(\Cset^{(0)})$ from
\Cref{thm:global-solution-quality}. See \Cref{appendix:proof-merge-final-bound} for the complete proof.
\end{proof}

\begin{remark}[Interpretation and Practical Implications]
\Cref{thm:merge-final-bound} completes the theoretical chain from partitioning to the final output:
\begin{itemize}[leftmargin=*]
    \item The term $\sum_{i,j}\epsilon_{(i,j)}$ captures the local suboptimality, which can be controlled by the quality of the local co-clustering algorithm.
    \item The term $\lambda(K_0-1)$ explicitly bounds the error introduced by merging. The regularization parameter $\lambda$ can be tuned via cross-validation or MDL principles to balance reconstruction accuracy and model complexity.
    \item The bound is constructive: each component is computable or controllable, making the framework amenable to sensitivity analysis.
\end{itemize}
This result addresses the theoretical gap identified in the review process: we now have an explicit, unconditional bound on the final reconstruction error that accounts for all stages of the algorithm—partitioning, local optimization, and merging.
\end{remark}

\subsection{Summary of Theoretical Guarantees}
\label{subsec:theory-summary}
The theoretical analysis establishes a complete chain of guarantees for DiMergeCo:
\begin{enumerate}[leftmargin=*]
    \item \textbf{Co-Cluster Preservation} (\Cref{thm:co-cluster-preservation}): Probabilistic partitioning ensures that significant co-clusters are preserved with exponentially high probability.
    \item \textbf{Local-to-Global Quality} (\Cref{thm:global-solution-quality}): The union of local solutions provides a bounded approximation to the global optimum.
    \item \textbf{Merging Convergence} (\Cref{lem:merge-monotone}): Hierarchical merging monotonically optimizes a regularized objective and terminates in finite time.
    \item \textbf{Final Error Bound} (\Cref{thm:merge-final-bound}): The final solution satisfies an explicit reconstruction error bound accounting for all algorithmic stages.
\end{enumerate}
These results demonstrate that DiMergeCo is not a heuristic divide-and-conquer method but a principled framework with rigorous quality guarantees.
...
{\color{blue}
\paragraph{Discussion on Regularization Parameter $\lambda$}
The regularization parameter $\lambda$ in \eqref{eq:regularized-objective} controls the trade-off between reconstruction accuracy and model simplicity. In practice, $\lambda$ can be selected via:
\begin{itemize}[leftmargin=*]
    \item \textbf{Cross-validation:} Choose $\lambda$ that minimizes validation set error.
    \item \textbf{MDL principle:} Set $\lambda$ proportional to the encoding cost per co-cluster (e.g., $\lambda \propto \log(MN)$).
    \item \textbf{Heuristic scaling:} Use $\lambda = \alpha \cdot \bar{\sigma}^2$, where $\bar{\sigma}^2$ is the average within-cluster variance and $\alpha$ is a user-specified factor (e.g., $\alpha \in [0.01, 0.1]$).
\end{itemize}
The sensitivity analysis of $\lambda$ is provided in the experimental section, demonstrating that the framework is robust across a range of values.
}

\subsection{Practical Considerations}
\label{subsec:practical-considerations}
In summary, these theoretical results establish that DiMergeCo is a principled framework with end-to-end quality guarantees. It reliably preserves co-clusters during partitioning (\Cref{thm:co-cluster-preservation}), produces a global solution with bounded sub-optimality (\Cref{thm:global-solution-quality}), performs hierarchical merging with monotonic convergence (\Cref{lem:merge-monotone}), and achieves a final solution with explicit reconstruction error bounds (\Cref{thm:merge-final-bound}). The framework balances theoretical rigor with practical scalability, making it suitable for large-scale co-clustering applications.
}
...
To validate the effectiveness of our hierarchical merging strategy and support the theoretical analysis in \Cref{thm:merge-final-bound}, we conduct a comprehensive ablation study comparing five merging strategies:
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{} p{0.08\textwidth} p{0.35\textwidth} @{}}
        ...
        $I_i^{(t)}, J_j^{(t)}$             & Row and column blocks in trial $t$ with sizes $\phi_i$ and $\psi_j$, respectively             \\
        $R_k, C_k^{\mathrm{col}}$          & Row and column index sets for co-cluster $C_k$, respectively                                  \\
        $X_{i,t}^{(k)}, Y_{j,t}^{(k)}$     & Row/column hit counts: $|R_k \cap I_i^{(t)}|$ and $|C_k^{\mathrm{col}} \cap J_j^{(t)}|$      \\
        $M_{(i,j)}^{(k)}, N_{(i,j)}^{(k)}$ & Row and column sizes of co-cluster $C_k$ in block $\mathbf{B}_{(i,j)}$, respectively          \\
        $s_i^{(k)}, t_j^{(k)}$             & Row/column gap parameters with positive part for Hoeffding bound                              \\
        $F_{k,t}$                          & Miss event for co-cluster $k$ in trial $t$                                                    \\
        $q_k, q_{\max}$                    & Single-trial miss probability for co-cluster $k$ and maximum over all co-clusters             \\
        $\bar p_{\mathrm{row}}^{(k)}, \bar p_{\mathrm{col}}^{(k)}$ & Row/column failure probability products over blocks                           \\
        $\gamma_k, \gamma_{\min}$          & Decay rates: $\gamma_k = -\log q_k$, $\gamma_{\min} = \min_k \gamma_k$                        \\
        $p_{\mathrm{det}}(\text{all})$     & Probability of detecting all $K$ co-clusters over $T_p$ trials                                \\
        ...
        $J(\Cset)$                         & Co-clustering reconstruction error objective function                                         \\
        $u_C$                              & Mean value of elements in co-cluster $C$                                                      \\
        $\lambda$                          & Regularization parameter for merging objective                                                \\
        $\widetilde J_\lambda(\Cset)$      & Regularized objective: $J(\Cset) + \lambda |\Cset|$                                          \\
        $\Delta J(C_a, C_b)$               & Merge cost for combining co-clusters $C_a$ and $C_b$                                          \\
        $P$                                & Number of processors in MPI implementation                                                    \\
        $\mathcal{B}_k$                    & Set of blocks overlapping with co-cluster $C_k$                                               \\
        $\Cset_{(i,j)}$                    & Local co-clusters discovered in block $(i,j)$                                                 \\
        $K_0$                              & Initial number of co-clusters before merging                                                  \\
        $\Cset^{(0)}$                      & Union of all local co-clusters before merging                                                 \\
        $\Cset_{\mathrm{merge}}$           & Final co-clustering result after hierarchical merging                                         \\
        \bottomrule
    \end{tabular}
\end{table}
...
\subsection{Proof of Hypergeometric Lower-Tail Bound}
\label{appendix:proof-hyp-tail}

\begin{proof}[Proof of \Cref{lem:hyp-tail}]
We prove \eqref{eq:row-tail}; the proof of \eqref{eq:col-tail} is identical.

Let the population values be $Z_1,\dots,Z_M\in\{0,1\}$ where
$Z_r=1$ if and only if $r\in R_k$. Then $\frac{1}{M}\sum_{r=1}^M Z_r = \frac{M^{(k)}}{M}$.
In trial $t$, the row block $I_i^{(t)}$ is a uniformly random subset of $[M]$
of size $\phi_i$ (obtained by random permutation and cutting, per Assumption~\ref{assump:perm-partition}), hence
\[
...
\le \exp(-2\phi_i\varepsilon^2).
\]
This bound holds because sampling without replacement exhibits \emph{negative correlation},
making the tail lighter than the corresponding with-replacement (Bernoulli) case.

Set $\varepsilon = s_i^{(k)} = \left(\frac{M^{(k)}}{M}-\frac{T_m-1}{\phi_i}\right)_+$.
Note that:
\begin{itemize}[leftmargin=*]
    \item If $\mu_i^{(k)} = \phi_i \cdot \frac{M^{(k)}}{M} \le T_m-1$, then $s_i^{(k)}=0$ and
    the bound $\exp(-2\phi_i(s_i^{(k)})^2)=1$ is trivial (but valid).
    \item If $\mu_i^{(k)} > T_m-1$, then $s_i^{(k)}>0$ and we have a non-trivial exponential bound.
\end{itemize}
...
\subsection{Proof of Single-Trial Miss Bound}
\label{appendix:proof-single-trial-miss}

\begin{proof}[Proof of \Cref{thm:single-trial-miss}]
By Lemma~\ref{lem:trial-equivalence}, the miss event in a single trial is
\[
...
For the first term, under Assumption~\ref{assump:NA} (negative association), and since $\{X_{i,t}^{(k)}<T_m\}$ are decreasing events,
\[
\Pr\Big(\bigcap_{i=1}^m \{X_{i,t}^{(k)}<T_m\}\Big)
\le \prod_{i=1}^m \Pr(X_{i,t}^{(k)}<T_m).
\]
By Lemma~\ref{lem:hyp-tail}, each term satisfies
$\Pr(X_{i,t}^{(k)}<T_m) \le \exp(-2\phi_i(s_i^{(k)})^2)$,
hence
\[
\Pr\Big(\bigcap_i \{X_{i,t}^{(k)}<T_m\}\Big)
\le \prod_{i=1}^m \exp\!\left(-2\phi_i (s_i^{(k)})^2\right)
= \exp\!\left(-2\sum_{i=1}^m \phi_i (s_i^{(k)})^2\right)
= \bar p_{\mathrm{row}}^{(k)}.
\]

Similarly for the column term,
\[
\Pr\Big(\bigcap_j \{Y_{j,t}^{(k)}<T_n\}\Big)
\le \bar p_{\mathrm{col}}^{(k)}.
\]

Combining gives \eqref{eq:single-trial-miss}.
\end{proof}

\subsection{Proof of Detecting All Co-Clusters Over $T_p$ Trials}
\label{appendix:proof-detect-all}

\begin{proof}[Proof of \Cref{thm:probability-co-cluster-detection}]
For a fixed co-cluster $k$, define the event $A_{k,t}:=\{\text{co-cluster }k\text{ is missed in trial }t\}=F_{k,t}$.
Then $\Pr(A_{k,t})=q_k$, where $q_k$ is the single-trial miss probability (trial-invariant by identical distribution).

By trial independence (Assumption~\ref{assump:trial-indep}),
\[
\Pr(\text{miss }k\text{ in all }T_p\text{ trials})
=\Pr\Big(\bigcap_{t=1}^{T_p} A_{k,t}\Big)=\prod_{t=1}^{T_p}\Pr(A_{k,t})=q_k^{T_p}.
\]

Let $B$ be the event ``miss at least one co-cluster over all trials''. Then
\[
...
\quad\text{where}\quad q_{\max}:=\max_{k\in[K]} q_k.
\]
...
Writing $q_{\max}^{T_p}=\exp(-(-\log q_{\max})T_p)$ and letting $\gamma_{\min}=-\log q_{\max}=\min_k(-\log q_k)$ gives \eqref{eq:detect-all-exp}.

Finally, Theorem~\ref{thm:single-trial-miss} provides the explicit upper bound
$q_k \le \bar p_{\mathrm{row}}^{(k)} + \bar p_{\mathrm{col}}^{(k)}$,
making all quantities computable.
\end{proof}
...
\subsection{Proof of Merge Cost Lemma}
\label{appendix:proof-merge-cost}

\begin{proof}[Proof of \Cref{lem:merge-cost}]
    We first expand the reconstruction error for the merged cluster $C_{ab} = C_a \cup C_b$:
    \begin{equation}
        J(\{C_{ab}\}) = \sum_{(i,j) \in C_{ab}} (a_{ij} - u_{C_{ab}})^2,
    \end{equation}
    where $u_{C_{ab}} = \frac{1}{|C_{ab}|}\sum_{(i,j) \in C_{ab}} a_{ij}$ is the mean of the merged cluster.

    Similarly, for the separate clusters:
    \begin{equation}
        J(\{C_a, C_b\}) = \sum_{(i,j) \in C_a} (a_{ij} - u_{C_a})^2 + \sum_{(i,j) \in C_b} (a_{ij} - u_{C_b})^2.
    \end{equation}

    The merge cost is:
    \begin{equation}
        \Delta J(C_a, C_b) = J(\{C_{ab}\}) - J(\{C_a, C_b\}).
    \end{equation}

    Using the parallel axis theorem (also known as Steiner's theorem or the variance decomposition formula), we can decompose the variance of the merged cluster. For disjoint sets $C_a$ and $C_b$, the within-cluster sum of squares for the merged cluster can be written as:
    \begin{equation}
        \begin{aligned}
            J(\{C_{ab}\}) &= \sum_{(i,j) \in C_a} (a_{ij} - u_{C_{ab}})^2 + \sum_{(i,j) \in C_b} (a_{ij} - u_{C_{ab}})^2 \\
            &= \sum_{(i,j) \in C_a} [(a_{ij} - u_{C_a}) + (u_{C_a} - u_{C_{ab}})]^2 \\
            &\quad + \sum_{(i,j) \in C_b} [(a_{ij} - u_{C_b}) + (u_{C_b} - u_{C_{ab}})]^2.
        \end{aligned}
    \end{equation}

    Expanding the squares and noting that $\sum_{(i,j) \in C_a} (a_{ij} - u_{C_a}) = 0$ and $\sum_{(i,j) \in C_b} (a_{ij} - u_{C_b}) = 0$ (by the definition of the mean), we obtain:
    \begin{equation}
        \begin{aligned}
            J(\{C_{ab}\}) &= J(\{C_a, C_b\}) + |C_a|(u_{C_a} - u_{C_{ab}})^2 + |C_b|(u_{C_b} - u_{C_{ab}})^2.
        \end{aligned}
    \end{equation}

    Therefore:
    \begin{equation}
        \Delta J(C_a, C_b) = |C_a|(u_{C_a} - u_{C_{ab}})^2 + |C_b|(u_{C_b} - u_{C_{ab}})^2.
    \end{equation}

    Since $u_{C_{ab}} = \frac{|C_a| u_{C_a} + |C_b| u_{C_b}}{|C_a| + |C_b|}$, we have:
    \begin{equation}
        u_{C_{ab}} - u_{C_a} = \frac{|C_b|}{|C_a| + |C_b|}(u_{C_b} - u_{C_a}), \quad
        u_{C_{ab}} - u_{C_b} = \frac{|C_a|}{|C_a| + |C_b|}(u_{C_a} - u_{C_b}).
    \end{equation}

    Substituting these into the expression for $\Delta J$:
    \begin{equation}
        \begin{aligned}
            \Delta J(C_a, C_b) &= |C_a| \left(\frac{|C_b|}{|C_a| + |C_b|}\right)^2 (u_{C_a} - u_{C_b})^2 \\
            &\quad + |C_b| \left(\frac{|C_a|}{|C_a| + |C_b|}\right)^2 (u_{C_a} - u_{C_b})^2 \\
            &= \frac{|C_a||C_b|^2 + |C_b||C_a|^2}{(|C_a| + |C_b|)^2} (u_{C_a} - u_{C_b})^2 \\
            &= \frac{|C_a||C_b|(|C_a| + |C_b|)}{(|C_a| + |C_b|)^2} (u_{C_a} - u_{C_b})^2 \\
            &= \frac{|C_a||C_b|}{|C_a| + |C_b|} (u_{C_a} - u_{C_b})^2 \ge 0.
        \end{aligned}
    \end{equation}
...
\subsection{Proof of Monotone Descent and Termination Lemma}
\label{appendix:proof-merge-monotone}

\begin{proof}[Proof of \Cref{lem:merge-monotone}]
    By construction, each merge is accepted only when $\widetilde J_\lambda(\Cset^{(t+1)}) \le \widetilde J_\lambda(\Cset^{(t)})$. This immediately establishes that the sequence $\{\widetilde J_\lambda(\Cset^{(t)})\}_{t \ge 0}$ is non-increasing.

    For termination, observe that each merge operation reduces the number of co-clusters by exactly one: if $\Cset^{(t)}$ contains $K_t$ co-clusters and we merge two of them to form $\Cset^{(t+1)}$, then $|\Cset^{(t+1)}| = K_t - 1$. Starting from $K_0 = |\Cset^{(0)}|$ co-clusters, the minimum possible number is $1$ (a single cluster containing all elements). Therefore, at most $K_0 - 1$ merges can occur.

    The algorithm terminates when no pair of co-clusters satisfies the merge criterion $\widetilde J_\lambda(\Cset^{(t+1)}) \le \widetilde J_\lambda(\Cset^{(t)})$. Since the number of possible merges is finite and each merge strictly reduces the number of clusters, the algorithm must terminate in finite time.
\end{proof}

\subsection{Proof of Global Objective Bound After Hierarchical Merging}
\label{appendix:proof-merge-final-bound}

\begin{proof}[Proof of \Cref{thm:merge-final-bound}]
    Let $\Cset^{(t)}$ denote the set of co-clusters after $t$ merges, with $\Cset^{(0)}$ being the initial union of local co-clusters. At each merge step $t$, we select two co-clusters $C_a, C_b \in \Cset^{(t)}$ and merge them into $C_{ab} = C_a \cup C_b$, producing $\Cset^{(t+1)} = (\Cset^{(t)} \setminus \{C_a, C_b\}) \cup \{C_{ab}\}$.

    The change in the regularized objective is:
    \begin{equation}
        \begin{aligned}
            \widetilde J_\lambda(\Cset^{(t+1)}) - \widetilde J_\lambda(\Cset^{(t)}) &= [J(\Cset^{(t+1)}) + \lambda|\Cset^{(t+1)}|] - [J(\Cset^{(t)}) + \lambda|\Cset^{(t)}|] \\
            &= [J(\Cset^{(t+1)}) - J(\Cset^{(t)})] + \lambda[|\Cset^{(t+1)}| - |\Cset^{(t)}|] \\
            &= \Delta J(C_a, C_b) - \lambda,
        \end{aligned}
    \end{equation}
    where the last equality uses \Cref{lem:merge-cost} and the fact that $|\Cset^{(t+1)}| = |\Cset^{(t)}| - 1$.

    Since each merge is accepted only when $\widetilde J_\lambda(\Cset^{(t+1)}) \le \widetilde J_\lambda(\Cset^{(t)})$, we have $\Delta J(C_a, C_b) \le \lambda$ for all accepted merges. Rearranging:
    \begin{equation}
        J(\Cset^{(t+1)}) - J(\Cset^{(t)}) = \Delta J(C_a, C_b) \le \lambda.
    \end{equation}

    Summing over all $T$ merges (where $T \le K_0 - 1$ by \Cref{lem:merge-monotone}):
    \begin{equation}
        J(\Cset_{\mathrm{merge}}) - J(\Cset^{(0)}) = \sum_{t=0}^{T-1} [J(\Cset^{(t+1)}) - J(\Cset^{(t)})] \le \sum_{t=0}^{T-1} \lambda = \lambda T \le \lambda(K_0 - 1).
    \end{equation}

    This proves \eqref{eq:merge-J-bound}. For \eqref{eq:merge-final-bound}, we substitute the bound from \Cref{thm:global-solution-quality}:
    \begin{equation}
        J(\Cset_{\mathrm{merge}}) \le J(\Cset^{(0)}) + \lambda(K_0 - 1) \le J(\Cset^*) + \sum_{i,j} \epsilon_{(i,j)} + \lambda(K_0 - 1).
    \end{equation}
