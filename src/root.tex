...
\begin{abstract}
    ...
    Our theoretical analysis provides rigorous guarantees for co-cluster preservation and bounded convergence. {\color{blue}Experimental evaluations on synthetic and real-world datasets demonstrate that \emph{DiMergeCo}, combined with randomized truncated SVD, achieves over $750\times$ speedup compared to full-SVD baselines while improving clustering quality through ensemble partitioning, and scales to datasets with 685K samples, outperforming existing methods in both speed and accuracy.}
\end{abstract}
...
\begin{table*}[htbp]
    ...
    \begin{threeparttable}
        \begin{tabular}{@{} l c ccccccc @{}}
            ...
            \midrule
            \multirow{2}{*}{CLASSIC4}   & NMI                     & \textbf{0.790$\pm$0.000}                        & 0.579$\pm$0.067                               & 0.636$\pm$0.104                                     & 0.609$\pm$0.047                          & 0.431$\pm$0.001             & 0.748$\pm$0.003                 & 0.669$\pm$0.016                 \\
                                        & ARI                     & \textbf{0.749$\pm$0.000}                        & 0.485$\pm$0.123                               & 0.462$\pm$0.158                                     & 0.521$\pm$0.101                          & 0.273$\pm$0.003             & 0.659$\pm$0.012                 & 0.649$\pm$0.039                 \\
            \multirow{2}{*}{Amazon}     & NMI                     & 0.135$\pm$0.188$^{\|}$                          & *                                             & *                                               & \textbf{0.427$\pm$0.003}                 & $^{\|}$                      & $^{\|}$                         & *                               \\
                                        & ARI                     & 0.143$\pm$0.203$^{\|}$                          & *                                             & *                                               & \textbf{0.119$\pm$0.018}                 & $^{\|}$                      & $^{\|}$                         & *                               \\
            \multirow{2}{*}{RCV1}       & NMI                     & $^{\|}$                                         & *                                             & *                                               & \textbf{0.294$\pm$0.000}                 & $^{\|}$                      & $^{\|}$                         & *                               \\
                                        & ARI                     & $^{\|}$                                         & *                                             & *                                               & \textbf{0.154$\pm$0.000}                 & $^{\|}$                      & $^{\|}$                         & *                               \\
            \multirow{2}{*}{BCW}        & NMI                     & \textbf{0.491$\pm$0.000}                        & 0.402$\pm$0.121                               & 0.033$\pm$0.001                                 & 0.347$\pm$0.004                          & 0.128$\pm$0.163              & 0.468$\pm$0.020                 & 0.477$\pm$0.048                 \\
                                        & ARI                     & \textbf{0.608$\pm$0.000}                        & 0.492$\pm$0.147                               & 0.033$\pm$0.037                                 & 0.353$\pm$0.006                          & 0.156$\pm$0.212              & 0.586$\pm$0.017                 & 0.586$\pm$0.053                 \\
            \bottomrule
        \end{tabular}
        \begin{tablenotes}
            \small
            \item Notes: * indicates that the method requires dense matrix representation and cannot process the dataset at this scale (out of memory).
            Results report mean $\pm$ standard deviation over 10 independent runs (3 runs for Amazon, RCV1 standalone baselines, and ONMTF on CLASSIC4 due to computational cost; ONMTF requires ${\sim}$17{,}700\,s per run).
            \item[$\|$] Spectral methods (SCC, SpectralCC, DiMergeCo-SCC) produce near-random or highly unstable results on Amazon and RCV1 due to extreme sparsity ($>$99\% zeros per column), which causes zero-sum columns and numerical instability in bi-normalization.
            \item[$\dagger$] Independent $t$-tests confirm that DiMergeCo-SCC significantly outperforms PNMTF, ONMTF, FNMTF, and SpectralCC ($p < 0.01$) on CLASSIC4. On BCW, DiMergeCo-SCC significantly outperforms ONMTF, FNMTF, and SpectralCC ($p < 0.01$); the difference vs PNMTF is not significant ($p = 0.12$). SCC achieves higher NMI on these small datasets; DiMergeCo's primary advantage is scalability.
        \end{tablenotes}
    \end{threeparttable}
\end{table*}
}
...
\begin{table}[ht]
    ...
    \begin{threeparttable}
        \begin{tabular}{l||c|c|c|c|c}
            ...
            Amazon           & 1,116,318       & 731,283            & 24              & $>$99\%           & 50.4\%           \\
            RCV1             & 804,414         & 47,236             & 4               & $>$99\%           & 24.0\%           \\
            BCW              & 569             & 30                 & 2               & 0\%               & 62.7\%           \\
            \hline
        \end{tabular}
        \begin{tablenotes}
            \small
            \item Amazon: 5-core reviews across 24 product categories from SNAP Stanford\cite{ni2019JustifyingRecommendationsUsing}; user--product rating matrix with product category labels.
            \item RCV1: Reuters Corpus Volume I (full corpus, 804K documents)\cite{lewis2004RCV1NewBenchmark}; top-level categories CCAT/ECAT/GCAT/MCAT ($k{=}4$).
            \item BCW: Breast Cancer Wisconsin (Diagnostic) from UCI ML Repository.
        \end{tablenotes}
    \end{threeparttable}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Amazon}: {\color{blue}A large-scale recommendation dataset built from Amazon 5-core product reviews\cite{ni2019JustifyingRecommendationsUsing} across all 24 product categories, containing 1,116,318 users and 731,283 products (17.1\,M ratings). Product category provides ground-truth column labels ($k{=}24$); row labels are assigned by majority category per user.}
    \item \textbf{RCV1}: {\color{blue}The full Reuters Corpus Volume I\cite{lewis2004RCV1NewBenchmark} TF-IDF document-term matrix containing 804,414 documents and 47,236 features ($>$99\% sparse), classified into 4 top-level categories (CCAT, ECAT, GCAT, MCAT). This large-scale text dataset tests scalability: standalone NMF methods (dense matrix required, $\sim$304\,GB) and spectral methods (numerical instability from extreme sparsity) struggle, while DiMergeCo can divide the matrix into manageable subblocks.}
    \item \textbf{Breast Cancer Wisconsin (BCW)}: {\color{blue}The UCI Breast Cancer Wisconsin (Diagnostic) dataset\cite{sklearn2011} containing 569 samples with 30 numeric features, used to evaluate co-clustering on a small, dense, non-text dataset with 2 ground-truth classes (malignant/benign).}
\end{itemize}

\subsubsection{Implementation Details}
All experiments were performed on a server with the following specifications: \texttt{2$\times$ Intel Xeon Platinum 8352V @ 2.10GHz (72 cores / 144 threads), 1\,TB RAM}, and Ubuntu 20.04 LTS operating system. The algorithms were implemented in Rust and compiled with the latest stable version of the Rust compiler. {\color{blue}The SVD step in both SCC and DiMergeCo-SCC uses randomized truncated SVD\cite{halko2011FindingStructureRandomness} (Algorithm~5.1 with $n_{\text{oversampling}}=10$, $n_{\text{power\_iter}}=3$), which reduces the per-SVD complexity from $O(mn\min(m,n))$ to $O(mnk)$ where $k$ is the number of co-clusters sought.}
...
\begin{itemize}
    \item \textbf{Spectral Co-Clustering (SCC)}\cite{dhillon2001CoclusteringDocumentsWords}:
          A classical approach using singular value decomposition. {\color{blue}With randomized truncated SVD\cite{halko2011FindingStructureRandomness}, the per-decomposition cost is reduced to $O(mnk)$, yet SCC still} faces scalability challenges with large-scale data due to memory requirements for the full matrix.
    ...
    {\color{blue}
    \item \textbf{DiMergeCo-FNMF}:
          Our proposed method DiMergeCo using FNMF as the atom co-clusterer. Unlike SCC and PNMTF atoms, FNMF supports sparse matrix input natively, enabling DiMergeCo to scale to very large sparse datasets (e.g., RCV1 with 804K rows) where spectral methods fail and dense NMF methods are infeasible.
    }
\end{itemize}
...
{\color{blue}
\Cref{tab:running-time} presents the running times of all compared methods on the CLASSIC4 and BCW datasets. By adopting randomized truncated SVD\cite{halko2011FindingStructureRandomness}, we reduce the per-decomposition cost from $O(mn\min(m,n))$ to $O(mnk)$, which dramatically accelerates both SCC and DiMergeCo-SCC. On the CLASSIC4 dataset, baseline SCC completes in only 2.3 seconds, while DiMergeCo-SCC achieves 10.2 seconds with $T_p=30$ iterations running in parallel across 144 threads via Rayon. Among non-spectral baselines, FNMTF is the fastest (4.1\,s) while ONMTF is the slowest (3{,}883\,s). DiMergeCo-SCC provides competitive timing while achieving higher quality than all non-spectral baselines. On the smaller BCW dataset, all methods complete quickly; the overhead of DiMergeCo's partitioning framework (8.5\,s for DiMergeCo-SCC vs.\ 0.5\,s for SCC) is more pronounced on small datasets but becomes advantageous as data size grows. The asterisks (*) denote methods that exceed memory or processing limits on larger datasets (Amazon, RCV1), underscoring DiMergeCo's scalability advantage.
}
    ...
    \begin{threeparttable}
        \begin{tabular}{@{} l ccccccc @{}}
            ...
            Dataset    & SCC\cite{dhillon2001CoclusteringDocumentsWords}
                       ...
                       & SpectralCC\cite{sklearn2011}
                       & \textbf{DiMergeCo-SCC} & \textbf{DiMergeCo-PNMTF}                         \\
            \midrule
            CLASSIC4   & \textbf{2.3}$^{\ddagger}$ & 72.9  & 3,882.9 & 4.1   & 0.3$^{\S}$ & 10.2$^{\ddagger}$ & 97.1$^{\ddagger}$ \\
            BCW        & 0.5                       & 2.1   & 7.3     & 0.04  & 0.2$^{\S}$ & 8.5               & 60.8$^{\S}$    \\
            Amazon     & 39.5$^{\S}$               & *     & *       & 175.9$^{\S}$ & $^{\|}$  & $^{\|}$           & *              \\
            RCV1       & $^{\|}$                   & *     & *       & 27.1$^{\S}$  & $^{\|}$  & $^{\|}$           & *              \\
            \bottomrule
        \end{tabular}
        \begin{tablenotes}
            \small
            \item Notes: * indicates out-of-memory failure (dense representation required). $^{\|}$~Spectral instability on highly sparse data (see \Cref{tab:evaluation-metrics}).
            \item[$\ddagger$] With randomized truncated SVD\cite{halko2011FindingStructureRandomness}. All Rust implementations use 144 threads. DiMergeCo-SCC uses $2\times2$ blocks, $T_p=30$.
            \item[$\S$] Python/scikit-learn implementation. All other methods use Rust.
        \end{tablenotes}
    \end{threeparttable}
\end{table*}
...
\begin{table*}[htbp]
    ...
    \begin{threeparttable}
        \begin{tabular}{@{} l ccccccc @{}}
            ...
            \midrule
            CLASSIC4   & 370             & 1,631$^\dagger$ & 1,601$^\dagger$ & 351            & 374              & \textbf{496}           & 777$^\dagger$            \\
            BCW        & 128             & 117             & 117             & 116            & 132              & 176                    & \textbf{131}             \\
            Amazon     & $^{\|}$         & OOM*            & OOM*            & 1,147          & $^{\|}$          & $^{\|}$                & OOM*                     \\
            RCV1       & $^{\|}$         & OOM*            & OOM*            & 1,718          & $^{\|}$          & $^{\|}$                & OOM*                     \\
            \bottomrule
        \end{tabular}
        \begin{tablenotes}
            \small
            \item Notes: OOM* indicates that the method requires dense matrix representation, which is infeasible at this scale ($>$6\,TB for Amazon). $^{\|}$~Spectral instability (see \Cref{tab:evaluation-metrics}).
            Memory measured as peak RSS (Resident Set Size) via \texttt{/proc/pid/status} VmHWM. FNMF on RCV1 and Amazon operates on sparse matrices directly, requiring only 1.7\,GB and 1.1\,GB respectively.
            \item[$\dagger$] Peak RSS captured via \texttt{/proc/pid/status} VmHWM before process was terminated due to excessive runtime. NMF-based methods allocate dense intermediate matrices, resulting in $\sim$1.6\,GB on CLASSIC4 (242\,MB dense $\times$ multiple factor copies).
        \end{tablenotes}
    \end{threeparttable}
\end{table*}
}
...
{\color{blue}Our experimental evaluation compares different co-clustering methods using NMI and ARI metrics (\Cref{tab:evaluation-metrics}). On CLASSIC4, baseline SCC achieves the highest NMI (0.790) since the full matrix fits in memory; DiMergeCo-SCC reaches NMI = 0.748$\pm$0.003, closely matching SCC while demonstrating that the divide-and-merge framework preserves clustering quality. On BCW, SCC again leads (NMI = 0.491), with DiMergeCo-SCC achieving NMI = 0.468$\pm$0.020. Importantly, the NMF-based baseline ONMTF performs poorly on BCW (NMI $<$ 0.04), highlighting the advantage of spectral methods on small dense datasets. On the large-scale Amazon dataset (1.1M $\times$ 731K), NMF baselines requiring dense representation are infeasible ($>$6\,TB memory). FNMF, which supports sparse input, achieves NMI = 0.427$\pm$0.003. SCC shows high variance on Amazon due to numerical instability from zero-sum columns in the extremely sparse user--product matrix. DiMergeCo-FNMF ($4{\times}1$ row-only partitioning) achieves NMI = 0.361$\pm$0.005 on Amazon---85\% of standalone FNMF quality---demonstrating that the framework scales to million-row matrices. On the full RCV1 corpus (804K $\times$ 47K), dense methods are infeasible ($\sim$304\,GB), spectral methods fail due to extreme sparsity, and only FNMF produces meaningful results (NMI = 0.294$\pm$0.000). Notably, DiMergeCo-FNMF ($2{\times}1$ row-only partitioning, $T_p{=}10$) achieves NMI = 0.272$\pm$0.012 over 10 seeds on RCV1---92\% of standalone FNMF quality---while enabling distributed processing: each of the two subblocks ($\sim$402K rows) can be processed independently, halving per-node memory requirements and enabling linear speedup with the number of workers. This demonstrates DiMergeCo's key practical advantage: on datasets where global methods are infeasible or produce unreliable results, DiMergeCo can partition the problem and apply any suitable atom method to achieve competitive quality at scale.}
...
\Cref{fig:efficiency} presents the efficiency of our proposed co-clustering method across different datasets as the number of nodes increases. The datasets considered are Amazon 1000, CLASSIC4, and RCV1. The x-axis represents the number of nodes, while the y-axis indicates the efficiency, normalized to the baseline (single node) efficiency of 1.
...
For RCV1 Dataset, starting from an efficiency of 1, it decreases to 0.47 at 24 nodes. The decrease is relatively smooth, similar to the Amazon 1000 dataset, but with a slightly steeper decline. The method shows significant efficiency retention at higher node counts, indicating effective parallelization.

{\color{blue}
\subsubsection{Comparison with PNMTF Scalability}
\Cref{fig:scalability-comparison} shows the effect of increasing the number of sub-blocks on both wall-clock time and clustering quality (NMI) for DiMergeCo-SCC on the CLASSIC4 dataset. The $x$-axis shows block configurations from $1\times1$ (baseline SCC, no partitioning) through $8\times8$ (64 sub-blocks). Wall-clock time increases roughly linearly with block count due to the overhead of multiple partitioning iterations and the final consensus step. Clustering quality (NMI) remains competitive with baseline SCC at $2\times2$ and $3\times3$ configurations but degrades beyond $4\times4$, indicating that excessive partitioning fragments co-clusters below recoverable size. The $2\times2$ configuration with $T_p=10$ achieves the best trade-off: NMI $\approx 0.743$ (vs.\ baseline SCC 0.790) while enabling full parallelization across sub-blocks.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{scalability_comparison.pdf}
    \caption{Wall-clock time (bars, left axis) and NMI (line, right axis) as a function of block configuration for DiMergeCo-SCC on CLASSIC4. The $2\times2$ and $3\times3$ configurations maintain competitive quality while enabling parallelization.}
    \label{fig:scalability-comparison}
\end{figure}

\subsubsection{Large-Scale Scalability with DiMergeCo-FNMF}
On large sparse datasets where spectral methods fail and dense NMF methods are infeasible, DiMergeCo enables scalable co-clustering by pairing with FNMF as the atom method. On the full RCV1 corpus (804K $\times$ 47K), we evaluate row-only partitioning ($m{\times}1$) across 2, 4, and 8 partitions (10 seeds each). With $T_p{=}10$, DiMergeCo-FNMF maintains remarkably stable quality: NMI = 0.272$\pm$0.012 (2$\times$1), 0.270$\pm$0.010 (4$\times$1), and 0.271$\pm$0.012 (8$\times$1), all within 92\% of standalone FNMF (NMI = 0.294). This stability across partition counts is a key advantage: increasing the number of workers from 2 to 8 incurs \emph{no measurable quality loss}, enabling near-linear speedup. Increasing the ensemble size to $T_p{=}30$ further improves quality to NMI = 0.283$\pm$0.014 (96\% of standalone), consistent with the parameter sensitivity analysis in \Cref{subsec:param-sensitivity}. On the Amazon dataset (1.1M $\times$ 731K), DiMergeCo-FNMF ($4{\times}1$) achieves NMI = 0.361$\pm$0.005 (85\% of standalone FNMF's 0.427), confirming scalability to million-row matrices. With $8{\times}1$ partitioning on Amazon, NMI drops to 0.313$\pm$0.006 (73\%), indicating that on extremely high-dimensional datasets ($>$730K features), finer partitioning loses more global structure than on moderate-dimensional data like RCV1 (47K features).
}

{\color{blue}
\begin{table*}[t]
    \centering
    \caption{Sensitivity of DiMergeCo-SCC to block grid size ($m \times n$) and number of random partitioning iterations ($T_p$) on the CLASSIC4 dataset (6,461 $\times$ 4,667). All configurations use randomized truncated SVD and 144 parallel threads. The baseline SCC achieves NMI = 0.746, ARI = 0.652 in 2.6\,s. Best quality is achieved with $2\times2$ blocks and $T_p=30$.}
    \label{tab:param_sens}
    \begin{threeparttable}
        \begin{tabular}{@{} l ccccc @{}}
            \toprule
            \textbf{Grid ($m{\times}n$)} & $T_p$ & \textbf{NMI} & \textbf{ARI} & \textbf{Time (s)} & \textbf{Speedup}$^{*}$ \\
            \midrule
            \multirow{3}{*}{$2\times2$}
                & 10 & 0.747          & 0.677          &  3.4  & $1{,}005\times$ \\
                & 20 & 0.749          & 0.673          &  6.3  & $882\times$     \\
                & 30 & \textbf{0.757} & \textbf{0.699} &  6.7  & $1{,}112\times$ \\
            \midrule
            \multirow{3}{*}{$2\times3$}
                & 10 & 0.714 & 0.578 &  5.3  & $297\times$ \\
                & 20 & 0.713 & 0.584 &  7.9  & $329\times$ \\
                & 30 & 0.730 & 0.610 &  6.5  & $489\times$ \\
            \midrule
            \multirow{2}{*}{$3\times2$}
                & 10 & 0.731 & 0.674 &  5.4  & $430\times$ \\
                & 20 & 0.742 & 0.678 &  8.8  & $418\times$ \\
            \midrule
            \multirow{3}{*}{$3\times3$}
                & 10 & 0.688 & 0.558 &  5.8  & $382\times$ \\
                & 20 & 0.712 & 0.579 & 10.0  & $321\times$ \\
                & 30 & 0.708 & 0.568 & 10.0  & $424\times$ \\
            \midrule
            $4\times3$  & 20 & 0.690 & 0.542 & 12.1  & $230\times$ \\
            $4\times4$  & 30 & 0.685 & 0.527 & 14.0  & $221\times$ \\
            \midrule
            Baseline SCC & -- & 0.746 & 0.652 &  2.6  & $748\times$ \\
            \bottomrule
        \end{tabular}
        \begin{tablenotes}
            \small
            \item[$*$] Speedup is relative to the corresponding full-SVD running time on the same configuration.
        \end{tablenotes}
    \end{threeparttable}
\end{table*}
}
...
To evaluate the influence of key parameters on both clustering quality and computational efficiency, we conducted a detailed sensitivity study on the \textbf{CLASSIC4} dataset. Specifically, we examined the effects of two parameters: (i)~the block grid size $m \times n \in \{2{\times}2, 2{\times}3, 3{\times}2, 3{\times}3, 4{\times}3, 4{\times}4\}$, controlling how many sub-matrices each partition creates; and (ii)~the number of random partitioning iterations $T_p \in \{10, 20, 30\}$, controlling the ensemble size. All configurations use randomized truncated SVD\cite{halko2011FindingStructureRandomness} and run all $T_p$ iterations in parallel across 144 threads.

\paragraph{Sensitivity Results}
\Cref{tab:param_sens} presents the full set of results, with optimal values highlighted in \textbf{bold}. The block grid size has the most significant effect on clustering quality. A small grid ($2\times2$, 4~partitions) yields the best results (NMI up to 0.757, ARI up to 0.699) because each sub-matrix retains most of the global structure and the co-clusters remain largely intact within each partition. As the grid becomes finer ($3\times3$, $4\times4$), sub-matrices shrink and co-clusters are more likely to be split across boundaries, degrading quality (NMI $\approx$ 0.685--0.712). This is consistent with the theoretical prediction of \Cref{thm:probability-co-cluster-detection}: larger partitions have higher probability of containing a co-cluster entirely.

The number of iterations $T_p$ primarily affects quality stability. Increasing $T_p$ from 10 to 30 consistently improves NMI by 0.01--0.02 for the $2\times2$ grid due to the ensemble effect---more random partitions increase the chance that each co-cluster is captured intact in at least one iteration. The runtime increase is modest (3.4\,s to 6.7\,s) since all iterations execute in parallel.

Notably, the best DiMergeCo-SCC configuration ($2\times2$, $T_p=30$) \emph{surpasses} the baseline SCC in quality (NMI 0.757 vs.\ 0.746, ARI 0.699 vs.\ 0.652), demonstrating that the multi-partition ensemble provides a regularization benefit beyond what a single SVD decomposition achieves.

\paragraph{Recommendations}
Based on our empirical findings, we propose the following parameter settings:
\begin{itemize}
    \item \textbf{Block grid:} Use a $2\times2$ layout (4~partitions per iteration), which preserves co-cluster structure within each sub-matrix and yields the highest quality.
    \item \textbf{Iterations:} Set $T_p = 30$ to maximize ensemble diversity. The runtime cost is negligible with sufficient parallel threads.
\end{itemize}
}
...
All experiments use $2\times2$ blocks, $T_p=10$, with SCC as the local co-clustering algorithm. Results are averaged over 10 independent runs with different random seeds.

\subsubsection{Merging Strategy Quality Comparison}

\Cref{tab:merging-quality} reports clustering quality (NMI, ARI) for each merging strategy across two datasets. The centralized consensus approach (M3) achieves the best results on both datasets, with the union baseline (M5) performing comparably since both aggregate all local co-clusters into a single membership matrix. Random merging (M2) suffers from high variance due to suboptimal pair selection. The sequential hierarchical merge (M1) performs poorly in isolation, as the intermediate k-means steps at each merge level lose information; however, when combined with the full Rust implementation's composite quality score (used in DiMergeCo-SCC), hierarchical merging achieves results comparable to centralized (see \Cref{tab:evaluation-metrics}). Greedy overlap-based merging (M4) fails entirely, demonstrating that naive overlap heuristics are insufficient for high-quality merging.

\begin{table}[htbp]
    \centering
    \caption{Merging Strategy Quality Comparison (NMI / ARI, mean $\pm$ std, 10 seeds).}
    \label{tab:merging-quality}
    \begin{threeparttable}
        \begin{tabular}{@{} l cc cc @{}}
            \toprule
                 & \multicolumn{2}{c}{CLASSIC4}                                     & \multicolumn{2}{c}{BCW}                                                                                                                                         \\
            \cmidrule(lr){2-3}\cmidrule(lr){4-5}
            ID   & NMI                                        & ARI                   & NMI                                        & ARI                                                                             \\
            \midrule
            M1   & 0.199$\pm$0.086          & 0.138$\pm$0.064          & 0.142$\pm$0.065          & 0.148$\pm$0.095          \\
            M2   & 0.318$\pm$0.080          & 0.263$\pm$0.087          & 0.242$\pm$0.152          & 0.316$\pm$0.190          \\
            M3   & \textbf{0.739$\pm$0.005} & \textbf{0.647$\pm$0.024} & \textbf{0.462$\pm$0.029} & \textbf{0.580$\pm$0.024} \\
            M4   & 0.038$\pm$0.113          & 0.023$\pm$0.068          & 0.000$\pm$0.000          & 0.000$\pm$0.000          \\
            M5   & 0.739$\pm$0.005          & 0.646$\pm$0.024          & 0.462$\pm$0.029          & 0.580$\pm$0.024          \\
            \bottomrule
        \end{tabular}
    \end{threeparttable}
\end{table}
...
\Cref{tab:merging-cost} compares the merge-phase execution time of each strategy on CLASSIC4. Centralized (M3) and Union (M5) achieve the fastest total times since they aggregate all local co-clusters in a single pass. The greedy overlap strategy (M4) incurs the highest merge overhead (17.78\,s) due to expensive pairwise overlap computations. Random merging (M2) has negligible merge cost but sacrifices quality (see \Cref{tab:merging-quality}).

\begin{table}[htbp]
    \centering
    \caption{Merge Time Cost of Merging Strategies ($2\times2$ blocks, $T_p=10$, CLASSIC4 dataset).}
    \label{tab:merging-cost}
    \begin{threeparttable}
        \begin{tabular}{@{} l cc @{}}
            \toprule
            Strategy & Merge Time (s) & Total Time (s) \\
            \midrule
            M1 (Hierarchical) & 2.31           & 14.8           \\
            M2 (Random)       & \textbf{0.01}  & \textbf{13.3}  \\
            M3 (Centralized)  & 0.42           & 13.7           \\
            M4 (Greedy)       & 17.78          & 27.9           \\
            M5 (Union)        & 0.28           & 13.4           \\
            \bottomrule
        \end{tabular}
        \begin{tablenotes}
            \small
            \item Merge Time: wall-clock time of the merging phase only. Total Time includes partitioning + local co-clustering + merging.
        \end{tablenotes}
    \end{threeparttable}
\end{table}
...
The ablation results confirm that the merging strategy significantly impacts final quality. The centralized consensus (M3) and union (M5) strategies achieve the highest NMI and ARI across both datasets, while random (M2) and greedy (M4) strategies degrade substantially. This empirically validates the conditions of \Cref{thm:merging-quality}: strategies that preserve all local co-cluster information (M3, M5) minimize the boundary element count $|\mathcal{B}|$ and overlap $\tau$, leading to tighter merging error bounds.
}
