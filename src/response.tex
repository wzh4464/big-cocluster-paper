...
\AR{We have added a new memory profiling table. \Cref{tab:memory-usage} reports peak RSS (Resident Set Size) for available methods across all four datasets. On the CLASSIC4 dataset, SCC and SpectralCC each consume 363\,MB, while DiMergeCo-SCC uses 482\,MB due to the membership matrix overhead from multiple partition iterations. The key scalability advantage of DiMergeCo is that per-node memory remains bounded as dataset size grows, since each node processes only its assigned submatrix. Methods marked ``OOM'' exceeded the system RAM limit.}
...
\textbf{Section~6 (Experiments):} Added \Cref{subsec:ablation} ``Ablation Study: Merging Strategy'' with three components: (a)~\Cref{tab:merging-quality} comparing NMI/ARI for 5 strategies $\times$ 2 datasets; (b)~\Cref{tab:merging-cost} comparing merge time costs; (c)~\Cref{fig:merging-convergence} showing objective function convergence.
\end{quote}
